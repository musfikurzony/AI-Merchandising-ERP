...
import { setViewer } from "./lib/viewerLens.js";
...
function FloatingSignOut({ signOut }) {
  ...
  /* /factory moved inside ErpShell in v96, and the shell carries its own
     Sign Out in the sidebar footer — rendering this one there too would
     overlap the header. Only /admin still has no header of its own. */
  const needsIt = pathname.startsWith("/admin");
...
export default function App() {
  ...
  /* Set before anything renders, because the data layer reads it. Done here
     rather than in useSession so it is impossible to have a route tree
     mounted against a profile the lens has not been told about. */
  setViewer(profile);
    ...
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route element={<ErpShell profile={profile} signOut={signOut} />}>
          ...
          {/* The Factory Portal lives INSIDE the shell now.

              v95 gave it its own bare layout and locked factory users out of
              everything else, on the reasoning that an outside company should
              not be inside the ERP. The owner corrected that: factory people
              are sometimes given the Workbench deliberately, so they can
              update their own milestones, and taking that away to hide a date
              removes a working practice to solve a different problem.

              So access goes back to the permission grid, the same as for
              anyone else — and the DATE is what gets protected instead, in
              lib/viewerLens.js, on every screen rather than only this one.
              Putting the portal in the shell also gives it the sidebar,
              header and search every other screen has, which is the other
